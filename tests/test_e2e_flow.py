...
def test_e2e_full_flow(tmp_path: Path):
    ...
    # Step 1 — init
    r = _mcp("devboard_init", project_root=root)
    assert r["status"] == "initialized"
    assert (tmp_path / ".devboard").exists()

    # Step 2 — add goal
    r = _mcp("devboard_add_goal", project_root=root,
    ...
    for i, step in enumerate(_decide_json()["atomic_steps"], start=1):
        ...
        _mcp("devboard_log_decision", project_root=root, task_id=task_id,
        ...
        # iter diff (simulated)
        _mcp("devboard_save_iter_diff", project_root=root, task_id=task_id,
    ...
    # (skill would call devboard_get_diff_stats first, see nothing, and skip)
    ...
    decisions = _mcp("devboard_load_decisions", project_root=root, task_id=task_id)
...
def test_e2e_replay_flow(tmp_path: Path):
    ...
    # Replay from iter 1
    r = _mcp("devboard_replay", project_root=root,
...
def test_e2e_iron_law_detector():
    ...
    r = _mcp("devboard_check_iron_law", tool_calls=[
...
def test_e2e_watch_sees_run(tmp_path: Path):
    ...
    runs = _mcp("devboard_list_runs", project_root=root)
...
def test_e2e_learnings_lifecycle(tmp_path: Path):
    ...
    _mcp("devboard_save_learning", project_root=root,
         name="zero_div_guard",
    ...
    results = _mcp("devboard_search_learnings", project_root=root,
                   query="division")
    assert len(results) == 1
    assert results[0]["confidence"] == 0.85

    # Retriever picks it up for a goal description
    r = _mcp("devboard_relevant_learnings", project_root=root,
...
def test_e2e_command_safety_gate(tmp_path: Path):
    ...
    r = _mcp("devboard_check_command_safety", command="rm -rf /")
    assert r["level"] == "block"

    r = _mcp("devboard_check_command_safety", command="git push --force")
    assert r["level"] == "warn"

    r = _mcp("devboard_check_command_safety", command="pytest tests/")
    assert r["level"] == "safe"
