...
def test_cso_runs_on_security_diff_and_can_block(
    ...
    mock_diff.return_value = "+ def login(user):\n+   session = create_session()"
    mock_planner.return_value = AgentResult("plan", [], _r("plan"))
    mock_executor.return_value = AgentResult("exec", [], _r("exec"))
    # Reviewer passes both iterations
    mock_reviewer.side_effect = [
        (ReviewVerdict.pass_, AgentResult("PASS", [], _r("PASS"))),
        (ReviewVerdict.pass_, AgentResult("PASS", [], _r("PASS"))),
    ]
    # First CSO: VULNERABLE → flips to RETRY. Second: SECURE → commit.
    mock_cso.side_effect = [
        (False, AgentResult("Verdict: VULNERABLE — session fixation", [], _r("VUL"))),
        (True, AgentResult("Verdict: SECURE", [], _r("SEC"))),
    ]
    rj = {"root_cause": "x", "next_strategy": "y", "learning": "", "risk": "MEDIUM",
          "risk_reason": "", "escalate": False, "phases": {}}
    mock_sysdebug.return_value = (rj, AgentResult("r", [], _r("r")))
    mock_verify.return_value = VerificationReport(full_suite_passed=True, full_suite_exit=0)

    goal = Goal(title="Auth", description="auth")
    store.save_goal(goal)

    result = run_loop(
        goal_id=goal.id, task_id="t_cso", goal_description="auth",
        locked_plan=plan, project_root=tmp_path, store=store,
        client=MagicMock(), enable_redteam=False, enable_cso=True,
    )
...
def test_cso_skipped_on_non_security_diff(
    ...
    # Innocent diff — no security keywords
    mock_diff.return_value = "+ def add(a, b):\n+   return a + b"
    mock_planner.return_value = AgentResult("plan", [], _r("plan"))
    mock_executor.return_value = AgentResult("exec", [], _r("exec"))
    mock_reviewer.return_value = (ReviewVerdict.pass_, AgentResult("PASS", [], _r("PASS")))
    mock_verify.return_value = VerificationReport(full_suite_passed=True, full_suite_exit=0)

    goal = Goal(title="Add", description="add")
    store.save_goal(goal)

    result = run_loop(
        goal_id=goal.id, task_id="t_nocso", goal_description="add",
        locked_plan=plan, project_root=tmp_path, store=store,
        client=MagicMock(), enable_redteam=False, enable_cso=True,
    )

    assert result.converged
    assert mock_cso.call_count == 0  # never called
