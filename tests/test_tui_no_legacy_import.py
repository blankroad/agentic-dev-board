...
def test_no_orchestrator_interrupt_import_in_new_app() -> None:
    """src/agentboard/tui/app.py (v2.0) must not import the LEGACY
    orchestrator.interrupt module. The legacy path lives in app_legacy.py
    only."""
    app_py = Path(__file__).resolve().parent.parent / "src" / "agentboard" / "tui" / "app.py"
    source = app_py.read_text()
    assert "agentboard.orchestrator.interrupt" not in source, (
