"""Validation that devboard_checkpoint warns on out-of-order events and
...
def _seed(tmp_path: Path) -> tuple[str, str, str]:
    ...
    r = _mcp("devboard_add_goal", project_root=root, title="T", description="t")
    goal_id = r["goal_id"]
...
def test_green_without_red_warns(tmp_path: Path):
    ...
    r = _mcp("devboard_checkpoint", project_root=str(tmp_path), run_id=run_id,
...
def test_converged_finds_task_id_from_prior_run_start(tmp_path: Path):
    ...
    goal_id, task_id, run_id = _seed(tmp_path)
    # run_start event already has task_id from devboard_start_task
    r = _mcp("devboard_checkpoint", project_root=str(tmp_path), run_id=run_id,
             event="tdd_complete", state={})
    r = _mcp("devboard_checkpoint", project_root=str(tmp_path), run_id=run_id,
