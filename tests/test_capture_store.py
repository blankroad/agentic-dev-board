...
def test_save_tui_capture_writes_file_with_iso_ts_stem(tmp_path: Path) -> None:
    ...
    (tmp_path / ".agentboard" / "goals" / "g_x").mkdir(parents=True)
...
def test_save_tui_capture_creates_captures_dir(tmp_path: Path) -> None:
    ...
    (tmp_path / ".agentboard" / "goals" / "g_y").mkdir(parents=True)
    assert not (tmp_path / ".agentboard" / "goals" / "g_y" / "captures").exists()
    save_tui_capture(tmp_path, "g_y", {})
    assert (tmp_path / ".agentboard" / "goals" / "g_y" / "captures").is_dir()
