...
description: D1a (2026-04-23). The single scope authority. **Scoped to agentboard-initialized projects only** (requires `.agentboard/` + `.mcp.json`; if absent, do NOT invoke this skill — use `superpowers:brainstorming` or gstack `office-hours` instead). Turns a user prompt into (a) a confirmed numbered request list, (b) a chosen alternative between 가장 이상적 / 현실적 / Approach C, and (c) a structured YAML-frontmatter `brainstorm.md` that downstream phase skills (frame / architecture / stress / lock) read verbatim — they NEVER re-decide scope. Replaces `agentboard-brainstorm` under D3 cutover. Paired with `--deep=ceo` (plan-ceo-review rubric) or `--deep=officehours` (YC 6 forcing questions).
