...
description: D1a (2026-04-23). The single scope authority. Turns a user prompt into (a) a confirmed numbered request list, (b) a chosen alternative between 가장 이상적 / 현실적 / Approach C, and (c) a structured YAML-frontmatter `brainstorm.md` that downstream phase skills (frame / architecture / stress / lock) read verbatim — they NEVER re-decide scope. Replaces `agentboard-brainstorm` under D3 cutover. Pre-cutover, invoke only when explicitly asked ("run intent", "direction interrogator", "clarify this idea"). Paired with `--deep=ceo` (plan-ceo-review rubric) or `--deep=officehours` (YC 6 forcing questions).
when_to_use: After D3 cutover, fires on any vague / short (<30 chars) / multi-request prompt. Pre-cutover, invoke only when the user explicitly asks for it. Voice triggers — "intent", "clarify this", "brainstorm this", "help me think through this". The sibling `agentboard-brainstorm` stays routed by default until the cutover; do NOT auto-invoke intent instead of brainstorm yet.
---
...
2. Call `agentboard_relevant_learnings(project_root, goal_description=<user prompt>)` and keep the top-5 for Phase 3 context injection (especially `constraint` + `pattern` categories). If empty: note "no prior learnings".
3. Run `Grep` / `Glob` on codebase for files/paths the user's prompt hints at. Prepare a 1-line summary per hit for Phase 3 context ("참고: {file:line — summary}"). If nothing found, note "no existing code found".
...
- **Exactly one request** → proceed to Phase 2 CLEAR Fast-Path check. `req_list` will have 1 entry with `status=in_scope`.
- **Multiple requests** → CLEAR is NOT eligible. Phase 3 adaptive loop adds a `wedge` axis. Non-wedge requests carry `status=deferred` in `req_list` and are called `후속 goal candidate` (not "non-goal") in prose. Downstream `agentboard-frame` / `agentboard-lock` treat deferred items as explicit non-goals.
...
Skip to Phase 4. At Phase 6 save, `alternatives_considered` may be 2 entries (이상적 always present + 현실적 collapsed to the obvious approach) since the space is already narrow.
...
After presenting alternatives + RECOMMENDATION, emit `AskUserQuestion` offering the 2-3 labels + "RECOMMENDATION 그대로 (Approach X)" as choices. Wait for explicit user selection. The chosen alternative drives `scope_mode` derivation at Phase 6:

- User picks 이상적 → `scope_mode = "EXPAND"`
- User picks 현실적 → `scope_mode = "HOLD"` (default) or `"REDUCE"` if the chosen alternative explicitly cuts scope below the original prompt
- User picks Approach C → derive case-by-case (often `"SELECTIVE"`)

Record the user's selection in Phase 6 `alternatives_considered` frontmatter with `chosen: true` on exactly one entry — `save_brainstorm` rejects 0-or-≥2 chosen (ValueError).

If the user declines to choose ("너가 정해줘" / "recommendation 그대로"): mark the RECOMMENDATION-pointed alternative as chosen AND log `phase="intent"`, `verdict_source="RECOMMENDATION_AUTOACCEPTED"` to flag that `user_confirmed = false` for downstream visibility.
...
1. **Placeholder scan** — `premises`, `alternatives`, and `rationale` must not contain any of: `TBD`, `(미기재)`, `{{...}}`, `<name>`, literal `...` as a field value.
...
### MCP call: `agentboard_save_brainstorm` (frontmatter MANDATORY)

Unlike legacy `agentboard-brainstorm`, the structured frontmatter fields are **not optional** here — downstream phase skills (`agentboard-frame`, `agentboard-architecture`, `agentboard-lock`) treat them as the sole scope authority. Emitting prose-only output from this skill is a spec violation.

#### Legacy prose fields (required — readable body)
...
#### Structured frontmatter fields (MANDATORY)

| Parameter | Derivation |
|---|---|
| `scope_mode` | `"EXPAND"` / `"SELECTIVE"` / `"HOLD"` / `"REDUCE"`. Derive from Phase 4 user selection: 이상적 → `EXPAND`, 현실적 → `HOLD` (or `REDUCE` if visibly cuts scope), Approach C → case-by-case (often `SELECTIVE`). Phase 5 self-review verifies alignment. |
| `refined_goal` | 1-sentence actionable goal reflecting the chosen alternative. Must be concrete enough that a developer can start immediately — no abstract nouns. |
| `wedge` | Narrowest concrete thing that would prove progress. From Phase 3 `wedge` axis (multi-request) or 1-sentence distillation of the chosen alternative (single-request / CLEAR path). |
| `req_list` | Phase 1 REQ items as `[{id: "R1", text: "...", status: "in_scope" \| "deferred"}, ...]`. Every Phase 1 item must appear. |
| `alternatives_considered` | Phase 4 entries as `[{name: "가장 이상적" \| "현실적" \| "Approach C", summary: "...", chosen: bool}, ...]`. **Exactly one** `chosen: true` — `save_brainstorm` raises `ValueError` otherwise (F4 contract). |
| `rationale` | 1-2 sentences: why the chosen alternative beats the others. Reference Phase 3 answers / constraints. |
| `user_confirmed` | `true` if Phase 1 restatement + Phase 4 selection were both explicitly confirmed. `false` only if the skill defaulted to RECOMMENDATION without explicit user pick — must also log `verdict_source="RECOMMENDATION_AUTOACCEPTED"`. |
...
agentboard_save_learning(
  ...
  tags=["intent", "<topic>"], category="constraint", confidence=0.6,
)
```

### Handoff (to the D1 chain, NOT gauntlet)
...
If the user refuses with "지금은 아니야" / "나중에" / equivalent: save only, exit without invoking frame.

**Do NOT invoke `agentboard-gauntlet`** — the legacy chain is frozen (see `memory/feedback_freeze_gauntlet_flow.md`). Even if a downstream phase skill is not yet fully filled (frame / architecture / stress / lock are all currently SKELETON in places), the routing chain is intent → frame.
...
- Phase 1 Request Restatement MANDATORY — deep modes do NOT skip it.
- Phase 4 MANDATORY slots (이상적 + 현실적) remain.
...
| "frame 스킵하고 바로 lock" | D1 체인은 순차 — frame 스킵은 assumptions 누락으로 이어짐. 거부. |
...
| Phase 0 — inject prior lessons | `agentboard_relevant_learnings(project_root, goal_description)` |
| Phase 4 — RECOMMENDATION auto-accepted | `agentboard_log_decision(phase="intent", verdict_source="RECOMMENDATION_AUTOACCEPTED")` |
| Phase 5 — audit trail | `agentboard_log_decision(phase="self_review", verdict_source="PASSED"\|"WARNING")` |
| Phase 6 — save | `agentboard_save_brainstorm(project_root, goal_id, premises, risks, alternatives, existing_code_notes, scope_mode, refined_goal, wedge, req_list, alternatives_considered, rationale, user_confirmed)` |
| Phase 6 — reusable constraint (optional) | `agentboard_save_learning(...)` — `category="constraint"`, `confidence=0.6` |
| Handoff — D1 chain ready | `agentboard_log_decision(phase="intent", verdict_source="COMMITTED", reasoning=<chosen alt + scope_mode>)` |

---

## Design notes (why this structure)

- **Phase 1 Request Restatement** is kept verbatim from `agentboard-brainstorm` — it catches multi-request prompts that would otherwise silently collapse to a single wedge. This is agentboard-specific value.
- **YAML frontmatter MANDATORY** (not optional as in legacy brainstorm) — downstream D1 skills parse the structured fields, not the prose body. Prose body is for humans; frontmatter is for machines.
- **Handoff is `agentboard-frame`, not `agentboard-gauntlet`** — this skill is part of the D1 chain that replaces the frozen gauntlet flow. Legacy brainstorm still hands off to gauntlet for backward compat; intent does not.
- **`scope_mode` derivation from user's Phase 4 pick is mechanical** — no extra LLM reasoning. The single scope decision is the user's confirmed alternative.
- **`--deep` modes are opt-in** — default path remains lightweight 6 phases. CEO / officehours add depth only when the user explicitly asks. This prevents the "every goal runs CEO review" scope creep that would accumulate token cost.
- **Hard cap 3** (Phase 3) prevents interrogation fatigue. Up to 3 questions + `ASSUMPTION` records for anything still unclear, then move on.
- **Self-review (Phase 5)** borrowed from obra/superpowers — placeholder / distinctness / actionable-risk + new frontmatter-completeness + scope-alignment checks catch the "draft looks fine but has `TBD`" class of bug seen in prior goals.
...
Per `memory/feedback_freeze_gauntlet_flow.md`, default skill routing still points at `agentboard-brainstorm` (legacy). Users can invoke `agentboard-intent` explicitly today; default auto-routing migrates here at D3 cutover. Once D3 lands, `agentboard-brainstorm` is deprecated and this file becomes the canonical scope-authority skill.
