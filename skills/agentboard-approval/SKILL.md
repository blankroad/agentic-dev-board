...
description: Final approval gate before git push + PR — **scoped to agentboard-initialized projects only** (requires `.agentboard/` + `.mcp.json` in repo root; if absent, do NOT invoke this skill and let the regular git push flow proceed). When the project IS agentboard-initialized, proactively invoke (do NOT git push directly) when the user says "ship it", "ship", "open a PR", "merge this", "push it", "deploy", "make a PR", "land this", OR after loop convergence (reviewer PASS + CSO SECURE + red-team SURVIVED + all checklist items verified). Summarizes diff stats, goal checklist verification, iteration stats, and key decisions. Prompts user for squash policy (squash/semantic/preserve/interactive). Builds PR body from LockedPlan + decisions automatically. Creates PR via `gh pr create`. NEVER force-pushes. REFUSES to push if any checklist item unverified or CSO returned VULNERABLE.
when_to_use: Project has `.agentboard/` + `.mcp.json` AND user signals readiness to push/merge/ship. Automatic after agentboard-redteam SURVIVED (or after agentboard-tdd full green + checklist verified if red-team was skipped). Voice triggers (agentboard projects only) - "ship it", "land it", "open a PR", "push this up". In non-agentboard projects, this skill does NOT apply — fall through to the regular push flow.
---
...
On entry, call `agentboard_load_decisions(project_root, task_id)` and verify these conditions:
...
3. **Dep audit check** — call `agentboard_check_dependencies(project_root)`.
...
### Diff stats
Call MCP tool `agentboard_get_diff_stats(project_root)` → show file/lines summary.

### Checklist verification
Load LockedPlan via `agentboard_load_plan(goal_id)`. For each checklist item, mark ✓ (verified by `agentboard_verify`) or ✗ (must block PR).

### Iteration stats
Call `agentboard_load_decisions(task_id)`:
...
Show the constructed PR body (call `agentboard_build_pr_body(plan, decisions, iterations)`). Ask: "Approve and push? [y/N]"

If N: task stays in `awaiting_approval` state. User can edit manually.

## Step 3.5 — Smoke gate (if integration_test_command present)

At entry, check `integration_test_command` via `agentboard_load_plan`:

- If empty string, skip this step (output one line: "No smoke test defined — skipping gate.")
- If a command is present:
  1. Call `agentboard_check_command_safety(command)` first for safety
...
1. Call `agentboard_apply_squash_policy(project_root, branch, base_branch, policy, squash_message)` to reshape commits per policy
2. Call `agentboard_push_pr(project_root, branch, pr_title, pr_body, base_branch, draft)` which:
...
After this write, continue to Step 4.6 (real-TTY screenshot for UI tasks), then `agentboard_checkpoint "converged"` and `agentboard_update_task_status status="pushed"`.
...
| When | Tool | Notes |
|---|---|---|
| On entry | `agentboard_load_plan(project_root, goal_id)` + `agentboard_load_decisions(project_root, task_id)` | Build summary |
| Before asking user | `agentboard_get_diff_stats(project_root)` | |
| Before asking user | `agentboard_verify(project_root, checklist)` | Re-verify fresh |
| On user approval | `agentboard_apply_squash_policy(project_root, branch, base_branch, policy, message)` | |
| Build PR body | `agentboard_build_pr_body(project_root, goal_id, task_id, iterations_completed, diff_stats)` | |
| Push (if remote) | `agentboard_push_pr(project_root, branch, pr_title, pr_body, base_branch)` | |
| **Always** after push/commit | `agentboard_checkpoint(project_root, run_id, "converged", {pr_url_or_reason, policy, iterations})` | **MANDATORY — even if local-only ship** |
| **Always** after converged | `agentboard_update_task_status(project_root, task_id, "pushed")` | |
| **Always** after converged | `agentboard_log_decision(project_root, task_id, iter=N, phase="approval", reasoning=<pr_url_or_reason>, verdict_source="PUSHED")` | |
...
Skip entirely when `ui_surface=False`. The legacy `agentboard_tui_render_smoke` (crash gate) still runs independently.
