...
This skill's instructions are in English. Code, file paths, identifiers, MCP tool names, CLI flags, and model names stay English. **All other user-facing output (chat replies AND the written `report.md`) must be in Korean**, following the rules below.
