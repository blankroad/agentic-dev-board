...
This skill's instructions are in English. Code, file paths, identifiers, MCP tool names, CLI flags, and model names stay English. **All other user-facing output (chat replies AND the written `review_session.md`) must be in Korean**, following the rules below.
...
Failure → write to `.agentboard/goals/<goal_id>/review_session_draft.md` for debug, skip main save, log `NARRATIVE_SKIPPED` with `reason=<rule>`, return `{status: "skipped"}`.
...
Target: `.agentboard/goals/<goal_id>/review_session.md` (UTF-8, overwrite).
...
6. Skip if a learning with the same name already exists in `.agentboard/learnings/<name>.md`.
...
- Success: `{status: "generated", path: ".agentboard/goals/<goal_id>/review_session.md", learnings_saved: N}`
