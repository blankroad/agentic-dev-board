...
from devboard.models import DecisionEntry, Goal, LockedPlan, Task
...
def collect_doc(store: FileStore, goal_id: str) -> DesignDoc:
    ...
    gauntlet_dir = store.root / ".devboard" / "goals" / goal_id / "gauntlet"
    gauntlet_steps = {}
...
def _render_md(doc: DesignDoc) -> str:
    ...
    lines.append(f"- devboard goal: `{g.id}`")
    lines.append("")
    ...
    lines.append("---")
    lines.append(f"_Generated by devboard from `{g.id}`. Raw state: `.devboard/goals/{g.id}/`_")
